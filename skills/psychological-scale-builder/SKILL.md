...
description: 标准化心理测评量表构建工具，支持将离散的量表文本内容（题目、评分规则、结果解释等）自动转换为统一规范的JSON结构，内置临床心理学量表标准规范。使用场景：(1) 制作/转换心理测评量表，(2) 标准化量表数据结构，(3) 为心理测评Agent提供量表数据源，(4) 生成符合规范的量表JSON文件。
---
